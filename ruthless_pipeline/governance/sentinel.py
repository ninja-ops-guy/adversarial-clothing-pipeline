"""Sealed sentinel, calibration-isolation, and pipeline-bridge protocol."""
...
def _seed(value: int, field: str) -> None:
...
class SentinelCohort:
    ...
    def validate(self) -> None:
        _kind(self.sentinel_id, IdKind.SENTINEL, "sentinel_id")
        _kind(self.source_cohort_id, IdKind.COHORT, "source_cohort_id")
        _kind(self.experiment_id, IdKind.EXPERIMENT, "experiment_id")
        _kind(self.constraint_id, IdKind.CONSTRAINT, "constraint_id")
        ...
        _hash(self.selection_policy_hash, "selection_policy_hash")
...
class SentinelRegistry:
    """Append-only registry: one immutable sentinel selection per source wave."""
    ...
    def register(self, sentinel: SentinelCohort) -> None:
        ...
        if sentinel.source_wave_id in self._by_wave:
            raise SentinelGovernanceError(
                f"source wave already anchored by {self._by_wave[sentinel.source_wave_id]}; "
...
def create_sentinel(
    *, sentinel_id: str, source_wave_id: str, source_seal: CohortSeal,
    ...
    sentinel = SentinelCohort(
        sentinel_id, source_wave_id, source_seal.cohort_id, source_seal.experiment_id,
        source_seal.constraint_id, selected, selection_seed, selection_policy_hash,
        source_seal.manifest_hash,
    )
    sentinel.validate()
    return sentinel
...
class CalibrationState:
    ...
    def validate(self) -> None:
        _kind(self.calibration_id, IdKind.CALIBRATION, "calibration_id")
...
class EvaluationPipeline:
    ...
    def validate(self) -> None:
        _kind(self.pipeline_id, IdKind.PIPELINE, "pipeline_id")
        ...
        if self.preprocessing_mode is PreprocessingMode.STATELESS:
            ...
            _kind(self.calibration_id, IdKind.CALIBRATION, "calibration_id")
            _hash(self.calibration_state_hash, "calibration_state_hash")
        else:
    ...
    def fingerprint(self) -> tuple[object, ...]:
        self.validate()
        return (
            self.software_version, self.code_hash, self.detector_hash,
            self.preprocessing_hash, self.random_seed, self.preprocessing_mode.value,
...
def validate_blind_sentinel_evaluation(
    ...
    if pipeline.preprocessing_mode is PreprocessingMode.CALIBRATED:
        ...
        if calibration.calibration_id != pipeline.calibration_id or calibration.state_hash != pipeline.calibration_state_hash:
            raise SentinelGovernanceError("pipeline calibration binding does not match calibration state")
        ...
        if overlap:
            raise SentinelGovernanceError(
                "calibration set overlaps sentinel/bridge/treatment/held-out membership: " + ", ".join(sorted(overlap))
            )
...
class PipelineBridgePlan:
    ...
    def validate(self) -> None:
        _kind(self.bridge_id, IdKind.BRIDGE, "bridge_id")
        _kind(self.sentinel_id, IdKind.SENTINEL, "sentinel_id")
        _kind(self.old_pipeline_id, IdKind.PIPELINE, "old_pipeline_id")
        _kind(self.new_pipeline_id, IdKind.PIPELINE, "new_pipeline_id")
