...
def source():
    data = b"source"
    manifest = {
        "cohort_id": "RAC-COHORT-SOURCE01",
        "experiment_id": "RAC-EXP-SOURCE01",
        "constraint_id": "RAC-CS-OLD0001",
        "code_commit": "a" * 40,
        "seed": 17,
...
def test_sentinel_is_bound_to_sealed_source_and_resampling_is_forbidden():
    ...
    other = create_sentinel(
        sentinel_id="RAC-SENT-WAVE002", source_wave_id="WAVE-N", source_seal=seal,
...
def test_sentinel_rejects_specimen_outside_sealed_cohort():
    ...
    with pytest.raises(SentinelGovernanceError, match="subset"):
        create_sentinel(
            sentinel_id="RAC-SENT-WAVE001", source_wave_id="WAVE-N", source_seal=seal,
...
def test_blind_pipeline_rejects_prior_outcome_access():
    with pytest.raises(SentinelGovernanceError, match="prior-wave"):
        validate_blind_sentinel_evaluation(sentinel(), pipeline("RAC-PIPE-PIPE001", leak=True))


def test_calibration_is_disjoint_and_frozen():
    s = sentinel()
    cal = CalibrationState("RAC-CAL-CAL0001", ("C1", "C2"), H4, H1, 22)
    p = pipeline("RAC-PIPE-PIPE001", mode=PreprocessingMode.CALIBRATED, cal=cal.calibration_id, cal_hash=cal.state_hash)
    ...
    p2 = pipeline("RAC-PIPE-PIPE002", mode=PreprocessingMode.CALIBRATED, cal=contaminated.calibration_id, cal_hash=contaminated.state_hash)
...
def test_pipeline_change_requires_paired_bridge_on_same_sentinel():
    s = sentinel()
    old = pipeline("RAC-PIPE-PIPE001")
    new = pipeline("RAC-PIPE-PIPE002", detector=H4)
    ...
    bridge = PipelineBridgePlan("RAC-BRIDGE-PIPE001", s.sentinel_id, old.pipeline_id, new.pipeline_id)
    validate_pipeline_transition(old, new, sentinel=s, bridge=bridge)


def test_constraint_bridge_requires_intersection_membership():
    b = ConstraintBridgeCohort("RAC-BRIDGE-CST0001", "RAC-CS-OLD0001", "RAC-CS-NEW0001", ("A", "B"), "RAC-SAMP-BRIDGE1", "intersection/v1")
...
def test_law4_requires_overlap_bound_to_same_constraint_pair():
    with pytest.raises(OverlapGovernanceError, match="Law 4"):
        require_overlap_analysis_for_migration("RAC-CS-OLD0001", "RAC-CS-NEW0001", None)
...
def test_low_common_support_forces_regime_reset():
    ...
    analysis = OverlapAnalysis("RAC-OVERLAP-MIG0001", "RAC-CS-OLD0001", "RAC-CS-NEW0001", ("TARGET",), rows, policy_overlap, "mc+bootstrap")
    require_overlap_analysis_for_migration("RAC-CS-OLD0001", "RAC-CS-NEW0001", analysis)
...
def test_policy_shift_alone_can_force_regime_reset():
    ...
    analysis = OverlapAnalysis("RAC-OVERLAP-MIG0002", "RAC-CS-OLD0001", "RAC-CS-NEW0001", tuple(sorted(material)), rows, po, "fixture")
