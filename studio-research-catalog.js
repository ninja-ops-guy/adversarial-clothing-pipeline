...
function candidateMatchesFamily(family){return Boolean(importedCanvas&&importedMetadata?.research_family===family);}
...
function renderImportedOrPlaceholder(ctx,size,family){
  ...
  if(candidateMatchesFamily(family)){ctx.imageSmoothingEnabled=true;ctx.drawImage(importedCanvas,0,0,size,size);}
...
function resetCandidateState(statusText='NO CANDIDATE LOADED'){
  importedCanvas=null;importedMetadata=null;
  ...
  const input=document.getElementById('researchCandidateFile');if(input)input.value='';
  setResearchStatus(statusText);
}
function invalidateCandidateForFamily(family){
  if(importedMetadata&&!candidateMatchesFamily(family))resetCandidateState('CANDIDATE CLEARED · FAMILY CHANGED · REIMPORT REQUIRED');
}
function updateProductExportControls(family,research){
  ...
  for(const method of ['exportTile','exportMockup','exportManifest']){
    ...
    if(research&&!ready){if(!button.disabled)button.dataset.researchDisabled='true';button.disabled=true;button.title='Import a governed candidate for the selected P0 family before export.';}
...
function sanitizeResearchManifest(manifest,family){
  ...
  if(!frozen?.active||frozen.research_family!==family||!candidateMatchesFamily(family))throw new Error('P0 manifest export requires the active family-bound governed candidate');
  if(!/^[0-9a-f]{64}$/i.test(String(frozen.sha256||'')))throw new Error('P0 manifest export requires a SHA-256-bound candidate');
  copy.art_direction_profile='p0_governed_candidate_import_v1';
  copy.generation_mode='governed_candidate_import';
...
function installProductStudioGuards(){
  ...
  for(const method of ['exportTile','exportMockup']){
    ...
    studio[method]=function(...args){const family=activeFamily();if(isResearchFamily(family)&&!candidateMatchesFamily(family)){setResearchStatus('EXPORT BLOCKED · IMPORT FAMILY-BOUND GOVERNED CANDIDATE');return false;}return original(...args);};
  }
  if(originalExportManifest)studio.exportManifest=function(...args){
    ...
    if(!candidateMatchesFamily(family)){setResearchStatus('EXPORT BLOCKED · IMPORT FAMILY-BOUND GOVERNED CANDIDATE');return false;}
...
async function importCandidate(file){
  ...
  setResearchStatus('READING CANDIDATE…');
  try{
    ...
    if(window.RACProduction?.renderAll)window.RACProduction.renderAll();
    syncAll();setResearchStatus(`LOADED · ${file.name} · SHA-256 ${hash.slice(0,12)}…`);
  }catch(err){setResearchStatus('IMPORT ERROR · '+err.message);}
}
