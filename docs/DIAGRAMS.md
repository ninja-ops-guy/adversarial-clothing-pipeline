...
> Mermaid compatibility note: labels are deliberately quoted and use conservative ASCII syntax so GitHub's renderer and older Mermaid runtimes parse the same source reliably.
...
flowchart TB
    ...
    subgraph CP["Certification and Evidence Plane"]
      FM["Frozen contracts"]
...
```mermaid
flowchart LR
    A["Stage 0 landmark-free smoke PASS"] --> B["Stage 1: 8 families x 8 compositions"]
...
stateDiagram-v2
    ...
    DigitalEvaluated --> ProductionReady: governance path allows production
