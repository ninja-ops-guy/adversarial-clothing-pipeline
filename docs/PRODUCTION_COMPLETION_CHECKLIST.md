...
|---|---|---|---|
| 1 | **Close D2-0004.** Finish surrogate/adaptive selection, freeze one exact candidate, run the fresh held-out set once, retain the result regardless of PASS/FAIL, verify candidate SHA is identical across candidate → benchmark → Product Studio → print kit | **In progress** | HO-v3 frozen without inference; one-shot generation rule active; surrogate adaptive search validated separately; measured run must be re-triggered after the failed `744dccf` attempt. SHA continuity enforced by hash-bound frozen tiles (277cc0e) |
