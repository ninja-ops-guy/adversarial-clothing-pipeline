(function(){
  ...
  function snapshot(){
    ...
    state.reference_quality_gate={minimum_side_px:MIN_REFERENCE_SIDE,mode:'fail_closed'};
  ...
  function enforceReferenceQuality(){
    ...
    audit.onload=()=>{
      const minSide=Math.min(audit.naturalWidth||0,audit.naturalHeight||0);
      if(minSide<MIN_REFERENCE_SIDE){
        if(toggle){toggle.checked=false;toggle.disabled=true;toggle.title=`Rejected: source is ${audit.naturalWidth}×${audit.naturalHeight}; minimum side is ${MIN_REFERENCE_SIDE}px`;}
        img.src=vectorShowcase();
        img.alt='High-resolution RAC vector showcase generated by the Product Studio';
        addQualityBadge(frame,`LOW-RES SOURCE REJECTED · ${audit.naturalWidth}×${audit.naturalHeight} → 2K VECTOR PREVIEW`,'reject');
        const note=document.querySelector('.showcase-source-controls .mini-note');
        if(note)note.textContent=`The bundled raster reference was only ${audit.naturalWidth}×${audit.naturalHeight}px and is blocked from pattern sampling. A scalable 2048×1536 vector showcase is displayed instead. Measured detector evidence remains separate.`;
      }else{
        addQualityBadge(frame,`SOURCE ${audit.naturalWidth}×${audit.naturalHeight} · ACCEPTED`,'ok');
      }
    };
    audit.onerror=()=>{
      if(toggle){toggle.checked=false;toggle.disabled=true;}
      img.src=vectorShowcase();
      addQualityBadge(frame,'SOURCE INVALID · 2K VECTOR PREVIEW','reject');
    };
